...
import { GET } from "../app/api/resident/report/route";
...
function makeRequest() {
  return new Request("http://localhost/api/resident/report", { method: "GET" });
}

describe("GET /api/resident/report", () => {
  ...
  test("returns 403 when user is not a Resident", async () => {
  ...
  test("returns 200 when user is a Resident", async () => {
    ...
    expect(body.message).toBe("Resident endpoint ready");
  });
});